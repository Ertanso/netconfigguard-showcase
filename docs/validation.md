...
A separate detailed Turkish demonstration uses Ahmet Neural synthetic narration with the same captured application outputs. Its 20 timed sections explain each screen, rationale and scope; the 723.57-second export includes 107 timestamped subtitle sentences. Audio and video decoded completely without errors, and chapter timing was checked for narration overruns. Claims are limited to supported configuration analysis, not guaranteed discovery of every risk. See [detailed transcript](product-walkthrough.tr.md).

The long demonstration was revised with evidence immediately after the opening, burned-in Turkish captions, and an original synthesized ambient bed automatically attenuated during speech. The revised full audio/video export decoded without errors; original result screenshots remain the evidence source.
